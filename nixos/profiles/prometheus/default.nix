      ...
      - name: homelab.rules
        rules:
          ...
          - alert: HomelabTaskMetricsCountMismatch
            expr: |
              count(homelab_task_last_run_unix{task!~"^(hello|managed_state_restore|managed_dirs_rehydrate|dev_checkouts_sync)$"}) != 9
            labels:
              severity: critical
              rulegroup: homelabRules
            annotations:
              summary: "Homelab task metric count mismatch (expected 9, got {{ $value }})"
              description: |
